...
- `memoh-ios-dev.md` — the iOS client (`apps/mobile`): design baseline, Memoh protocol contract, and every change this fork makes to upstream. Read it before touching `apps/mobile/`, `tools/` or `infra/`.
...
### iOS Client (`apps/mobile`)

This fork adds an **iOS-only** client next to upstream's web and desktop apps. It is the fork's only deliverable; everything else in this repository is upstream and should stay untouched. Design, protocol contract and the full list of upstream deviations live in `memoh-ios-dev.md`.

- Expo Router + React Native, plus one local Expo Swift module. **No Android** — no implementation, no fallback stub, no Android build script.
- All first-party native capability and native UI lives in `apps/mobile/modules/memoh-kit` and is registered through `MemohKitModule`. Do not add a second bridge package. When RN cannot reach the UI bar, implement it in Swift/UIKit or SwiftUI — **do not imitate a system effect in RN**.
- `apps/mobile/ios` is generated by Expo prebuild and is gitignored. Changes belong in the app config, a local config plugin, or MemohKit — **never only in the generated project**.
- Keep normal Xcode signing on, including simulator validation. Do not use `CODE_SIGNING_ALLOWED=NO`.
- Credentials go through the Keychain (via MemohKit). **Never** copy tokens, passwords, session content or private service configuration into the repository.
- Gates: `pnpm ios:check`, `pnpm ios:test`, `pnpm ios:bundle`; after touching native code also `pnpm ios:verify:build` (or `pnpm ios:test:hosted`). Upstream's own `pnpm lint` and `pnpm test` must stay green too.
- Prefer **behavioural verification** over implementation snapshots. **Screenshots prove visual state, recordings prove temporal behaviour** — a screenshot alone is not evidence of visual correctness.
- Do not nest ternary expressions (same rule as upstream): one `cond ? a : b` is fine, another `?` inside either branch is not.
- Preserve the user's work. Before any destructive revert/restore/rollback, look at the working tree and get explicit confirmation.
- Never submit anything to upstream `felinics/Memoh`. Server-side changes go through our fork `AidenNovak/Memoh`.
...
Please refer to `./memoh-ios-dev.md`. It is the only iOS-side document in this repository — design baseline, protocol contract, upstream deviations and the iOS gates all live there.
