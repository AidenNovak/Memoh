...
- Credentials go through the Keychain via `expo-secure-store`. **Never** copy tokens, passwords, session content or private service configuration into the repository.
- Gates: `pnpm ios:check` and `pnpm ios:bundle`; after touching native code, build the generated Xcode project. Upstream's own `pnpm lint` and `pnpm test` must stay green too.
