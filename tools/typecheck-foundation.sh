...
# `MemohNotifications`——它们 `import UIKit`，只能在 iOS SDK 下查（`pnpm typecheck:kit`，
# 要 Xcode）。清单在 `docs/NEEDS-XCODE.md` §1.1。
#
# ## 与 `pnpm test:swift` 的重叠是故意的
#
...
echo "类型检查通过（UIKit 那 5 个不在这里：要 iOS SDK，见 docs/NEEDS-XCODE.md §1.1）"
